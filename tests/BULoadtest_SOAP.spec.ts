...
const csvdata = parse(fs.readFileSync(path.join(__dirname, "testdata/Vol_testData.csv")), {
    columns: true,
...
if (record.FeeSettings === 'true') {
    await page.locator('#mat-slide-toggle-2').click();

    // --- Instapay ---
    if (record.Instapay === 'true') {
        await page.locator('#mat-slide-toggle-4').click();

        if (record.iFixedFee === 'true') {
            await page.locator('#mat-slide-toggle-9').click();
            await page.getByRole('spinbutton', { name: 'Fixed Fee' }).fill(record.FixFee);
        }
        if (record.iMDR === 'true') {
            await page.locator('#mat-slide-toggle-10').click();
            // Use the ID to avoid ambiguity with the other MDR field
            await page.locator('#mdrFee').fill(record.MDR);
        }
    }

    // --- STARPAY to STARPAY Merchant ---
    if (record.S2SMerchant === 'true') {
        await page.locator('#mat-slide-toggle-5').click();

        if (record.S2SFixedFee === 'true') {
            await page.locator('#mat-slide-toggle-11').click();
            await page.locator('#spFee').fill(record.S2SFixFee);
        }

        if (record.S2SMDR === 'true') {
            await page.locator('#mat-slide-toggle-12').click();
            // FIX: Use the specific ID 'spMdrFee' to resolve the strict mode violation
            await page.locator('#spMdrFee').fill(record.S2SMDR2);
        }
    }
}
        await page.pause();
